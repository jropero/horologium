...
import { X, CalendarDays, BookOpen, Sun, Moon, Map } from 'lucide-react';
...
interface MayaCalendarModalProps {
  isOpen: boolean;
  onClose: () => void;
  startDate: Date;
}
...
const MayaCalendarModal: React.FC<MayaCalendarModalProps> = ({ isOpen, onClose, startDate }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4 bg-black/90 backdrop-blur-md animate-fadeIn" onClick={onClose}>
      <div className="bg-ink border-4 border-emerald-600 rounded-xl shadow-2xl w-full max-w-4xl max-h-[95vh] flex flex-col relative overflow-hidden" onClick={e => e.stopPropagation()}>
        
        {/* Header */}
        <div className="bg-emerald-950/40 p-4 border-b-2 border-emerald-500/50 flex justify-between items-center sticky top-0 z-20 shadow-md">
          <div className="flex items-center gap-3 text-emerald-400">
            <BookOpen className="w-6 h-6" />
            <h2 className="font-serif text-xl font-bold uppercase tracking-widest text-parchment drop-shadow-md">
              Academia del Tiempo Maya
            </h2>
          </div>
          <button onClick={onClose} className="text-parchment/80 hover:text-red-400 hover:bg-white/10 transition-colors p-2 rounded-full">
            <X className="w-6 h-6" />
          </button>
        </div>

        {/* Tabs */}
        <div className="flex w-full bg-ink/80 border-b border-gold-dim/20 sticky top-[74px] z-10 shrink-0 overflow-x-auto no-scrollbar">
          <button
        ...
        {/* Content Area */}
        <div className="overflow-y-auto p-4 sm:p-6 custom-scrollbar bg-ink flex-1">
            ...
            <div className="animate-fadeIn space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                {TZOLKIN_LORE.map((t, i) => (
                  <div key={i} className="bg-ink border border-gold-dim/20 p-3 rounded-lg hover:border-emerald-500/50 hover:bg-emerald-900/20 transition-all text-center group cursor-pointer">
                    <div className="text-2xl mb-1 group-hover:scale-125 transition-transform">{t.icon}</div>
                    <div className="font-serif font-bold text-emerald-300 text-sm uppercase">{t.name}</div>
                    <div className="text-[10px] text-parchment/80 font-bold tracking-wider mb-2">{t.meaning}</div>
            ...
            <div className="animate-fadeIn space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {HAAB_NAMES.map((name, i) => {
                  ...
                  return (
                    <div key={i} className={`p-3 rounded-lg border flex flex-col items-center text-center transition-all cursor-pointer ${isWayeb ? 'bg-red-900/20 border-red-500/40 hover:bg-red-900/40' : 'bg-ink border-amber-500/20 hover:border-amber-500/50 hover:bg-amber-900/20'}`}>
                      <div className="text-2xl mb-1">{lore?.icon}</div>
                      <div className={`font-serif font-bold text-sm uppercase ${isWayeb ? 'text-red-400' : 'text-amber-300'}`}>{name}</div>
                      <div className={`text-[10px] font-bold tracking-wider mb-2 ${isWayeb ? 'text-red-300' : 'text-parchment/80'}`}>{lore?.title}</div>
          ...
          {activeTab === 'longcount' && (
            <div className="animate-fadeIn space-y-6">
              ...
              <div className="flex flex-col gap-4 max-w-2xl mx-auto">
                <div className="bg-blue-900/10 border border-blue-500/30 p-4 rounded-xl flex items-center justify-between group hover:bg-blue-900/30 transition-all">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-blue-500/20 flex items-center justify-center text-xl border border-blue-400/50">1</div>
                    <div>
                      <div className="text-xl font-serif font-black text-blue-300 uppercase tracking-widest">K'in</div>
                      <div className="text-sm text-parchment/80 italic">Día</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-black text-parchment">1 <span className="text-xs font-normal text-gold-dim">día</span></div>
                  </div>
                </div>

                <div className="bg-blue-900/10 border border-blue-500/30 p-4 rounded-xl flex items-center justify-between group hover:bg-blue-900/30 transition-all">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-blue-500/20 flex items-center justify-center text-xl border border-blue-400/50">20</div>
                    <div>
                      <div className="text-xl font-serif font-black text-blue-300 uppercase tracking-widest">Winal</div>
                      <div className="text-sm text-parchment/80 italic">Mes de 20 días</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-black text-parchment">20 <span className="text-xs font-normal text-gold-dim">días</span></div>
                  </div>
                </div>

                <div className="bg-blue-900/10 border border-blue-500/30 p-4 rounded-xl flex items-center justify-between group hover:bg-blue-900/30 transition-all">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-blue-500/20 flex items-center justify-center text-xl border border-blue-400/50">18</div>
                    <div>
                      <div className="text-xl font-serif font-black text-blue-300 uppercase tracking-widest">Tun</div>
                      <div className="text-sm text-parchment/80 italic">Año solar (aprox)</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-black text-parchment">360 <span className="text-xs font-normal text-gold-dim">días</span></div>
                  </div>
                </div>
                ...
                <div className="bg-blue-900/10 border border-blue-500/30 p-4 rounded-xl flex items-center justify-between group hover:bg-blue-900/30 transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-parchment/80 italic">Generación humana</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-black text-parchment">7,200 <span className="text-xs font-normal text-gold-dim">días</span></div>
                    <div className="text-xs text-blue-400/80">~ 19.7 años</div>
                  </div>
                </div>
                ...
                <div className="bg-blue-900/10 border border-blue-500/30 p-4 rounded-xl flex items-center justify-between group hover:bg-blue-900/30 transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-parchment/80 italic">Gran ciclo, era mundial</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-black text-parchment">144,000 <span className="text-xs font-normal text-gold-dim">días</span></div>
                    <div className="text-xs text-blue-400/80">~ 394 años</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'forecast' && (
            <div className="animate-fadeIn space-y-4">
              <div className="text-center mb-6">
                <h3 className="text-2xl font-black text-purple-400 font-serif mb-2">La Rueda del Tiempo Próximo</h3>
                <p className="text-parchment/90 font-serif text-sm">
                  Cómo se entrelazarán los engranajes del Tzolk'in y el Haab' en los siguientes 7 días.
                </p>
              </div>
              ...
              {nextDays.map((day, idx) => {
                const isToday = idx === 0;
                return (
                  <div key={idx} className={`p-4 rounded-xl border flex flex-col sm:flex-row gap-4 sm:items-center ${isToday ? 'bg-purple-900/20 border-purple-500/50 shadow-[0_0_15px_rgba(168,85,247,0.2)]' : day.isWayeb ? 'bg-red-900/20 border-red-500/40' : 'bg-ink border-gold-dim/20'}`}>
                    <div className="flex flex-col sm:w-28 shrink-0 border-b sm:border-b-0 sm:border-r border-gold-dim/20 pb-2 sm:pb-0">
                      <span className="text-parchment font-bold uppercase">{day.gregorianDate.toLocaleDateString('es-ES', { month: 'short', day: 'numeric' })}</span>
                      {isToday && <span className="text-[10px] bg-purple-500/20 text-purple-300 px-2 py-0.5 rounded-full mt-1 w-max uppercase font-bold tracking-wider">Hoy</span>}
                    </div>

                    <div className="flex-1 flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
                      {/* Tzolkin Info */}
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-emerald-900/30 border border-emerald-500/40 flex items-center justify-center text-emerald-300 font-black text-lg">
                          {day.tzolkin.number}
                        </div>
                        <div>
                          <div className="text-xs uppercase tracking-widest text-emerald-500/80 font-bold">Tzolk'in</div>
                          <div className="font-serif font-black text-lg text-parchment uppercase">{day.tzolkin.name}</div>
                        </div>
                      </div>

                      <div className="hidden sm:block text-2xl text-gold-dim/30">/</div>

                      {/* Haab Info */}
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-lg ${day.isWayeb ? 'bg-red-900/30 border border-red-500/40 text-red-400' : 'bg-amber-900/30 border border-amber-500/40 text-amber-300'}`}>
                          {day.haab.number}
                        </div>
                        <div>
                          <div className={`text-xs uppercase tracking-widest font-bold ${day.isWayeb ? 'text-red-400/80' : 'text-amber-500/80'}`}>Haab'</div>
                          <div className={`font-serif font-black text-lg uppercase ${day.isWayeb ? 'text-red-300' : 'text-parchment'}`}>{day.haab.name}</div>
                        </div>
                      </div>
                    </div>
                  </div>
...
export default MayaCalendarModal;
