...
const formatAtticDay = (dayOfMonth: number, monthLength: number): { short: string; full: string } => {
  if (dayOfMonth === 1) {
    return { short: "Νουμηνία", full: "Νουμηνία (Luna Nueva)" };
  }
  ...
  if (dayOfMonth <= 10) {
    ...
    return {
      ...
      full: `Ἡμέρα ${ordinal} τοῦ μηνὸς ἱσταμένου (Día ${dayOfMonth} de la luna creciente)`
    };
  }
  ...
  if (dayOfMonth <= 20) {
    ...
    return {
      ...
      full: `Ἡμέρα ${ordinal} ἐπὶ δέκα (Día ${dayOfMonth}, mitad del mes)`
    };
  }
  ...
  if (daysFromEnd === 1) {
    return { short: "ἕνη καὶ νέα", full: "Ἕνη καὶ Νέα (Último día, vieja y nueva luna)" };
  }
  const ordinal = GREEK_DAY_ORDINALS[daysFromEnd] || daysFromEnd.toString();
  return {
    short: `${ordinal} φθίνοντος`,
    full: `${ordinal} φθίνοντος (${daysFromEnd}º desde el fin del mes)`
  };
};
...
export const getAtticDate = (date: Date): AtticDateResult => {
  ...
  return {
    ...
    full: `${dayFormatted.full}, μηνὸς ${monthData.name} (${monthData.latin})`,
