...
export const getAtticFestivalInfo = (monthIndex: number, dayOfMonth: number): AtticFestivalInfo | null => {
  ...
  // 1. METAGEITNION (Finales de Verano)
  if (monthIndex === 1) {
    if (dayOfMonth === 7) return { festivalName: "Metageitnia", deity: "Apolo Metageitnios", deityDesc: "Apolo como dios del buen vecindario.", festivalDesc: "Festival homónimo del mes. Sacrificio y banquete comunitario para fortalecer los lazos entre vecinos (metageitnion = cambio de vecindario).", category: "Familiar y Social", aition: "El nombre recuerda la migración mítica de los jonios desde Melite a Diomea, guiados por Apolo, que les enseñó a ser buenos vecinos en su nuevo hogar." };
    if (dayOfMonth === 15) return { festivalName: "Herakleia en Kynosarges", deity: "Heracles", deityDesc: "Héroe deificado, patrón de los atletas.", festivalDesc: "Festival atlético celebrado en el gimnasio de Kynosarges, fuera de las murallas. Abierto a los nothoi (hijos ilegítimos) y metecos, a quienes normalmente se excluía.", category: "Cívico y Patriótico", participants: "Nothoi, Metecos, Atletas", agonDesc: "Competiciones atléticas en el gimnasio de Kynosarges. Era el único festival donde los hijos ilegítimos podían competir oficialmente.", aition: "Kynosarges ('perro blanco') recibe su nombre porque un perro blanco robó la carne del sacrificio a Heracles. El Oráculo dictaminó que el lugar donde el perro la devorase sería sagrado." };
  }
  ...
  if (monthIndex === 2) {
    ...
    if (dayOfMonth === 21) return { ...eleusisBase, festivalDayName: "Segundo día de revelaciones", festivalDesc: "Continuación de los ritos secretos en el Telesterion. Los epoptai (iniciados avanzados) recibían la visión suprema de las Cosas Sagradas.", pannychisDesc: "Segunda noche de vigilia sagrada. Los epoptai (los que ya habían sido iniciados antes) accedían al grado supremo de revelación.", aition: "El segundo día culminaba con la Epopteia: la visión definitiva que, según Plutarco, eliminaba el miedo a la muerte para siempre. Lo que se veía era tan secreto que revelarlo se castigaba con la muerte." };
  ...
  if (monthIndex === 3) {
    ...
    if (dayOfMonth === 19) return { festivalName: "Apaturia", festivalDayName: "Dorpia (El Banquete)", deity: "Zeus Phratrios y Atenea Phratria", deityDesc: "Dioses protectores de las fratrías.", festivalDesc: "Primer día de las Apaturias. Los miembros de cada fratría se reunían al anochecer para un banquete comunal costeado por las familias.", category: "Familiar y Social", participants: "Miembros de las Fratrías (clanes)", aition: "El nombre se asocia al 'engaño' (apate) porque Melanto venció al rey beocio Janto cuando Dioniso apareció tras Janto con una piel de cabra negra, distrayéndolo para el golpe mortal." };
    if (dayOfMonth === 20) return { festivalName: "Apaturia", festivalDayName: "Anarrhysis (El Sacrificio)", deity: "Dionisio Melanaigis", deityDesc: "Dionisio de la piel de cabra negra.", festivalDesc: "Segundo día. Gran sacrificio comunitario de la fratría a Zeus y Atenea. Los niños recitaban poemas.", category: "Familiar y Social", agonDesc: "Los niños recitaban poemas de memoria. El mejor recitador de cada fratría recibía un premio (según Platón, Timeo 21b).", aition: "La Anarrhysis conmemora el sacrificio que hizo Melanto tras su victoria engañosa: ofreció un macho cabrío negro para agradecer a Dioniso por su aparición fantasmal." };
    if (dayOfMonth === 21) return { festivalName: "Apaturia", festivalDayName: "Koureotis (El Corte de Pelo)", deity: "Zeus Phratrios", deityDesc: "Zeus protector de los clanes.", festivalDesc: "Tercer día y culminación. Los padres presentaban a sus hijos recién nacidos y los efebos recién iniciados ante la fratría para su inscripción oficial.", category: "Familiar y Social", participants: "Padres, Recién nacidos, Efebos", ritualOffering: { item: "Meion (ofrenda menor por recién nacidos) y Koureion (sacrificio mayor por efebos, con corte de pelo ritual)", icon: "✂️" }, aition: "El mechón de pelo cortado al efebo se ofrendaba a los dioses como símbolo de su paso de la infancia a la ciudadanía. Sin este registro en la fratría, un ateniense no podía heredar ni votar." };
  }

  // 4. MAIMAKTERION (Noviembre - Tormentas)
  if (monthIndex === 4) {
    if (dayOfMonth === 10) return { festivalName: "Maimakteria", deity: "Zeus Maimaktes", deityDesc: "Zeus el Tempestuoso, señor de las tormentas invernales.", festivalDesc: "Festival homónimo del mes. Sacrificio solemne de un carnero para apaciguar a Zeus y proteger los campos y las naves de las tormentas de noviembre.", category: "Purificación y Expiación", ritualOffering: { item: "Carnero sacrificado a Zeus Maimaktes. No se servía vino en las libaciones (festival nephálio)", icon: "🐏" }, aition: "El nombre Maimaktes significa 'el furioso'. Zeus recibía este epíteto porque el mes de noviembre traía las peores tormentas del Egeo, hundiendo barcos y arruinando cosechas tardías." };
    if (dayOfMonth === 20) return { festivalName: "Pompaia", deity: "Zeus Meilikhios", deityDesc: "Zeus el Apacible (en su aspecto ctónico).", festivalDesc: "Procesión purificatoria. Se paseaba el Dios Koidion (Vellón de Zeus) por la ciudad para absorber las impurezas acumuladas.", category: "Purificación y Expiación", ritualOffering: { item: "Dios Koidion (vellón sagrado del carnero sacrificado en la Maimakteria, usado como amuleto purificador)", icon: "🧶" }, aition: "El vellón del carnero sacrificado en la Maimakteria se convertía en un amuleto: quien lo pisara quedaba purificado. Se paseaba en procesión para que toda la ciudad se beneficiara del poder absorbente de la lana sagrada." };
  }
...
export const getDailyAtticDeity = (dayOfMonth: number, monthLength: number): AtticFestivalInfo | null => {
  ...
  const dailyDeities: Record<number, AtticFestivalInfo> = {
    1: { 
      festivalName: "Noumenia (Luna Nueva)",
      festivalDesc: "El día más afortunado del mes. Consagrado a los Dioses del Hogar.",
      deity: "Ἀπόλλων Ἀγυιεύς καὶ Ζεὺς Κτήσιος", 
      deityDesc: "Día de los Dioses del Hogar. La Luna Nueva bendice el umbral de la casa y purifica los nuevos comienzos bajo la protección de Apolo y Zeus.", 
    },
    2: { 
      festivalName: "Agathos Daimon",
      festivalDesc: "Día del 'Buen Espíritu' que protege a la familia.",
      deity: "Ἀγαθὸς Δαίμων", 
      deityDesc: "Día del Buen Espíritu. Se honra al protector invisible que habita en el hogar, brindando suerte y abundancia a través de la libación sagrada de vino puro.", 
    },
    3: { 
      festivalName: "Tritogeneia",
      festivalDesc: "Cumpleaños de Atenea, protectora de la sabiduría.",
      deity: "Ἀθηνᾶ Τριτογένεια", 
      deityDesc: "Aniversario mensual del nacimiento de Atenea. Una jornada para la sabiduría, la estrategia y el ingenio de la Diosa que guía el destino de la ciudad.", 
    },
    4: { 
      festivalName: "Tetras Hierai",
      festivalDesc: "Día de la fuerza, elocuencia y amor.",
      deity: "Ἡρακλῆς, Ἑρμῆς, Ἀφροδίτη καὶ Ἔρως", 
      deityDesc: "Día sagrado compartido por Heracles, Hermes, Afrodita y Eros. Una jornada poderosa para la fuerza, los viajes y el afecto.", 
    },
    5: { 
      festivalName: "Pempte Hierai",
      festivalDesc: "Día dedicado a Atenea y la justicia.",
      deity: "Ἀθηνᾶ", 
      deityDesc: "Día sagrado de Atenea. Una jornada para honrar la justicia, la laboriosidad y la protección divina sobre el hogar y la ciudad.", 
    },
    6: { 
      festivalName: "Artemisia",
      festivalDesc: "Cumpleaños de Artemisa. Sacrificios en su honor.",
      deity: "Ἄρτεμις", 
      deityDesc: "Aniversario mensual del nacimiento de Artemisa. Día para la pureza y los misterios de la naturaleza salvaje; se ofrecen sacrificios a la Señora de las Bestias.", 
    },
    7: { 
      festivalName: "Hebdome",
      festivalDesc: "Cumpleaños de Apolo. Día sagrado universal.",
      deity: "Ἀπόλλων", 
      deityDesc: "Aniversario mensual de Apolo. El día más luminoso del mes, consagrado a la armonía, la profecía y las artes de Febo. Un día sagrado en toda la Hélade.", 
    },
    8: { 
      festivalName: "Ogdoe",
      festivalDesc: "Día de Poseidón y Teseo.",
      deity: "Ποσειδῶν καὶ Θησεύς", 
      deityDesc: "Día del Mar y el Héroe. Consagrado al soberano de las aguas, Poseidón, y al mítico unificador de Ática, Teseo, celebrando la fuerza y el valor.", 
    },
    9: { 
      festivalName: "Enate",
